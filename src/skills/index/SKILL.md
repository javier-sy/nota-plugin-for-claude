...
After success, mention that the work now appears in `search` (kind: `"all"` or `"private_works"`) and `similar_works` results. Suggest using `{{cmd:analyze}}` to generate a deeper musical analysis of the work, or `{{cmd:code}}` to modify the composition.
...
- If the user asks about plugin configuration or API key issues, redirect to `{{cmd:setup}}`.
