...
- Changes to the framework affect future `{{cmd:think}}` sessions but do not retroactively change anything (ideation is ephemeral).
- The inspiration framework is independent from the analysis framework (`{{cmd:analysis-framework}}`). They serve different purposes and evolve separately.
